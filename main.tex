...
In computational complexity terminology, it has been known that the deciding whether an arbitrary multivariate polynomial is a \emph{nonnegative polynomial} (see Definition \ref{def:NGP}) is a NP-hard problem (computationally infeasible) when the 
degree of the polynomial is greater than two. 

However, if we redirect our focus to determine whether a given polynomial can be expressed in terms of \emph{sum of squares (SOS)} (see Definition \ref{def:SOS}),
the problem is reduced to solving a \emph{semidefinite programming (SDP)} (see Definition \ref{def:SDP}) problem which 
...
In this paper, we am going to run experiments, with the computer's aid, of different choices of the basis $\mathcal{B}$ to determine, 
...
The paper is organized in the following: In Section 2, we will introduce all the preliminary materials, 
...
And in Section 4, we will present some discussion about the result that is obtained in Section 3 and some thoughts on what are the further efforts that can be made to this problem.
...
In this section, we are going to introduce all the background materials that will be used to 
...
First, we would rigorously define the decision problem that is considered in this paper. 
Thus, we will define the terms that are used related to \emph{polynomials} in this subsection. 

Throughout this paper, we will use bold letters for vectors, e.g. $\mathbf{x} = (x_1, ..., x_n) \in \mathbb{R}^n$.
...
Some times, when the variables that we are working with is clear from the context, $\mathbb{R}[\mathbf{x}]_{n, d}$ will be abbreviated as $\mathbb{R}_{n, d}$.
...
Then, we will define the terminologies related to the decision problem.
...
Thus, we will devote this subsection introducing all the required tools. 

In order to properly define the \emph{SDP} and measure the stability of a linear system, we first introduce some tools from linear algebra.
...
By examining \eqref{eq:2-4}, we can see that we have a set of constraints (later we will see that the set of constraints can be translated exactly into the constraints in \emph{SDP}) 
